...
import { pageNumberPlus, pageNumberMinus, setPageNumber } from "../../../redux/actions/pagesActionCreator/actionCreator"
import { changeHitsPerPage } from '../../../redux/actions/hitsPerPageCreator/action';
...
const News = () => {
    ...
    const handleHitsChange = useCallback((e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        dispatchedSetPageNumber(0);
        dispatchedChangeHitsPerPage(+value);
    }, []);
    ...
    const handlePageChange = ({ target }: React.MouseEvent<HTMLButtonElement>) => {
        ...
        if(btnType && btnType.match(/[0-9]/)) { // проверка на содержание чисел
            dispatchedSetPageNumber(Number(btnType));
        } else {
            switch (btnType) {
                case 'next':
                    dispatchedPageNumberPlus();
                    break;
                case 'prev':
                    dispatchedPageNumberMinus();
                    break;
            }
        }
    }
